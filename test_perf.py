...
def run_shapes(n_iter, enable_profiler, enable_profiler_trace, check_accuracy, lib):
    ...
    # When both accuracy and profiler are enabled, we'll do accuracy check first with 1 iteration,
    # then performance profiling with n_iter iterations
    if enable_profiler_trace:
    ...
    shapes = [
        # K=16384 shapes - where we saw slight speedup before
        (((128, 2048), (2048, 1), 'torch.bfloat16'),
        ((128, 16384), (16384, 1), 'torch.float8_e4m3fn'),
        ((16384, 2048), (1, 16384), 'torch.float8_e4m3fn')),

        (((256, 2048), (2048, 1), 'torch.bfloat16'),
        ((256, 16384), (16384, 1), 'torch.float8_e4m3fn'),
        ((16384, 2048), (1, 16384), 'torch.float8_e4m3fn')),

        (((512, 1024), (1024, 1), 'torch.bfloat16'),
        ((512, 16384), (16384, 1), 'torch.float8_e4m3fn'),
        ((16384, 1024), (1, 16384), 'torch.float8_e4m3fn')),

        # Large M with moderate K
        (((1024, 2048), (2048, 1), 'torch.bfloat16'),
        ((1024, 8192), (8192, 1), 'torch.float8_e4m3fn'),
        ((8192, 2048), (1, 8192), 'torch.float8_e4m3fn')),

        (((2048, 1024), (1024, 1), 'torch.bfloat16'),
        ((2048, 8192), (8192, 1), 'torch.float8_e4m3fn'),
        ((8192, 1024), (1, 8192), 'torch.float8_e4m3fn')),

        # Small N with large K (accumulation-heavy)
        (((256, 512), (512, 1), 'torch.bfloat16'),
        ((256, 16384), (16384, 1), 'torch.float8_e4m3fn'),
        ((16384, 512), (1, 16384), 'torch.float8_e4m3fn')),
    ...
    if check_accuracy and enable_profiler:
        print(f"Will run accuracy check first, then performance benchmark with n_iter={n_iter}")
    elif check_accuracy:
        print(f"Running accuracy checks only")
    else:
        print(f"Running benchmarks with n_iter={n_iter}, profiler={'enabled' if enable_profiler else 'disabled'}")
    ...
    for i, shape_info in enumerate(shapes):
        ...
        # First run accuracy check if requested
        if check_accuracy:
            print(f"Running accuracy check for shape (m={m}, k={k}, n={n})...")
            run_fp8_mm_benchmark(m, k, n, libs=libs_to_run, n_iter=1, enable_profiler=False, enable_profiler_trace=False, check_accuracy=True)
        
        # Then run performance profiling if requested
        if enable_profiler or not check_accuracy:
            print(f"Running performance benchmark for shape (m={m}, k={k}, n={n})...")
            run_fp8_mm_benchmark(m, k, n, libs=libs_to_run, n_iter=n_iter, enable_profiler=enable_profiler, enable_profiler_trace=enable_profiler_trace, check_accuracy=False)
...
def main():
    ...
    parser.add_argument('--no-profiler', action='store_true',
                        help='Disable the PyTorch profiler. By default, profiler is enabled.')
    ...
    parser.add_argument('--no-accuracy-check', action='store_true',
                        help='Skip accuracy check against baseline. By default, accuracy check is performed.')
    parser.add_argument('--lib', type=str, default=None,
                        help='Kernel library to run. If not specified, all libraries will be run.')
    args = parser.parse_args()

    run_shapes(args.n_iter, not args.no_profiler, args.enable_profiler_trace, not args.no_accuracy_check, args.lib)
